...
bool baxter_core_msgs__msg__endpoint_states__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("baxter_core_msgs.msg._endpoint_states", (char *)PyUnicode_1BYTE_DATA(module_attr), 37) == 0);
    assert(strncmp("EndpointStates", (char *)PyUnicode_1BYTE_DATA(name_attr), 14) == 0);

    Py_DECREF(module_attr);
    Py_DECREF(name_attr);
    Py_DECREF(class_attr);
  }
  baxter_core_msgs__msg__EndpointStates * ros_message = _ros_message;
  {  // names
  ...
  {  // states
    ...
    if (!baxter_core_msgs__msg__EndpointState__Sequence__init(&(ros_message->states), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create baxter_core_msgs__msg__EndpointState__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    baxter_core_msgs__msg__EndpointState * dest = ros_message->states.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!baxter_core_msgs__msg__endpoint_state__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of EndpointStates */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("baxter_core_msgs.msg._endpoint_states");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "EndpointStates");
  ...
  {  // states
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = baxter_core_msgs__msg__endpoint_state__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
